...
All models were trained using a learning rate $\alpha$ of $0.001$, a regularization factor $\lambda$ of $0.01$ and a negative weight $w_n$ for negative instances of $0.5$, if not stated differently.
User and item features $p_u$ and $q_i$ are initialized using a Gaussian random distribution with mean zero and a standard deviation of $0.1$.
...
So at start 
%TODO explain the curve behave that differently.
% model errors do not correlate with performance. why? error curves actually suggest that lin works better up to a specific number of iterations
...
The presented prediction rules for the SVD and AdaBoost models are compared to each other over different numbers models as shown in figures \ref{fig:LogRegSVDPredictions} and \ref{fig:LinRegSVDPredictions}. %TODO insert plots over models and explain

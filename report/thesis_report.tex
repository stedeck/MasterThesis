...
Another user-user approach, that does not suffer from the above problem was implemented in \cite{aiolli2013efficient} or \cite{effectiveLatentModels}, see algorithm \ref{alg:CFUserNN2}.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\For{every item represented by a feature vector $x \in I \setminus I(u)$}
			...
			\If{$p_u(b=1|x) >min(recs)$ or $size(recs) < q$}
...
Both user-user approaches were implemented, though algorithm \ref{alg:CFUserNN2} requires a lot more runtime. %TODO add runtime requiremnts
...
\begin{figure}
	...
	\caption{Comparison of different prediction rules. Plots show model-based, user-based, aricle-based or combined prediction of user and article errors for five and one \text{LogRegSVD} model.}
	\begin{subfigure}[c]{1\textwidth}
...
Only the article-based prediction rule is able ro reach improvements in terms of the MAP when applied to a single \text{LogRegSVD} model for some numbers of iterations.
...
%todo add table with best performances for each approach
...
In those cases the article based prediction, see equation \ref{eq:articleErrorPrediction}, in some cases is able to outperform the traditional prediction.
%todo explain why
...
Tasks for future work are on one hand applying the algorithms to other e-commerce dataset to work out whether datasets of different dimensions or different domains achieve significantly different results.

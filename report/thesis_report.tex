...
\title{Multiple recommender systems for online shops}
\author{S.\ Deckers}
\id{DKE 17-04}
...
% Ensure floats land in the section where they are defined
...
So the prediction rule for AdaBoost was adapted to incorporate the errors for users or items instead of the total model error.
...
It contains 663,731 purchases, so it is only 0.06888\% dense, there are on average 56.37 purchases per item and 8.11 purchases per user. %todo make table out of stats
...
\appendix
	...
	\begin{figure}[h!]
		...
		\end{subfigure}
		\caption[Prediction rules for AdaBoost SVD over number of features]{Different prediction rules applied to 5 boosted SVD models over number of features. Models are trained with 2000 iterations, a learning rate $\alpha$ of 0.001, a regularization factor $\lambda$ of 0.01 and a weight $w_n$ of 0.5 for negative instances. User and item features $p_u$ and $q_i$ are initialized using a Gaussian random distribution with mean zero and a standard deviation of $0.1$.}
	\end{figure}
	\begin{figure}[h!]
		\begin{subfigure}[c]{1\textwidth}
		...
		\end{subfigure}
		\caption[Prediction rules for single SVD model over iterations]{Different prediction rules applied to a single SVD model over number of iterations. Models are trained with 300 features, a learning rate $\alpha$ of 0.001, a regularization factor $\lambda$ of 0.01 and a weight $w_n$ of 0.5 for negative instances.}
	\end{figure}
	\begin{figure}[h!]
		\begin{subfigure}[c]{1\textwidth}
			\centering
		...
		\end{subfigure}
		\caption[Prediction rules for AdaBoost SVD over iterations]{Different prediction rules applied to 5 boosted SVD models over number of iterations. Models are trained with 300 features, a learning rate $\alpha$ of 0.001, a regularization factor $\lambda$ of 0.01 and a weight $w_n$ of 0.5 for negative instances.}
	\end{figure}

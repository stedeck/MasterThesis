...
\begin{equation}
p_u^{(t)} = p_u^{(t)} + w_n \alpha d^{(t)}(u,i) (e_{ui}^{(t)} q_i^{(t)} + \lambda p_u^{(t)})
\end{equation}

and 

\begin{equation}
q_i^{(t)} = q_i^{(t)} + w_n \alpha d^{(t)}(u,i) (e_{ui}^{(t)} p_u^{(t)} + \lambda q_i^{(t)})
\end{equation}

where $d(u,i)$ denotes the weight for training instance of user $u$ for item $i$.
...
The trained model is evaluated using the training set and the weights for next model updated according to $d^{(t+1)}(u,i) = d^{(t)}(u,i) \exp(e_{ui})$, such that instances with a larger prediction errors will get larger feature updates when training the next model.
After all weights are updated they are normalized by $d^{t+1}(u,i) = \frac{d^{t+1}(u,i) |P_{train}|}{\sum_{(u,i)}{d^{t+1}(u,i)}}$, such that the sum of all instance weights adds up to the number of purchases again.
...
For recommendation this leads to following prediction rule to determine the score for a user- item-pair: %TODO see whether others used the same formula and add reference

\begin{equation}
	s(u,i) = \sum_t^T{\frac{ln(\frac{1 - \epsilon_t}{\epsilon_t})}{1 + exp((p_u^{(t)})^Tq_i^{(t)})} }
\end{equation} %TODO fix this... looks shitty
